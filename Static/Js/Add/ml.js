//Here we get the 2 inputs
let title = document.getElementById('title');
let inputMlSearch = document.getElementById('inputMlSearch');

let btnModelMl = document.getElementById('seeMl');

//Now we add an input event to title do fill its content to ml input

title.addEventListener('focusout', ()=> {

    //title.value = inputMlSearch.value;
    console.log('out');
...
//Now we send the heading to the back end

let mlSearchForm = document.getElementById('mlSearchForm');

let csrf = document.getElementsByName('csrfmiddlewaretoken');

let activeModalSearch = document.getElementById('activeModalSearch');


mlSearchForm.addEventListener('submit', (e)=> {

    e.preventDefault();

    let formData = new FormData();

    formData.append('title',inputMlSearch.value);
